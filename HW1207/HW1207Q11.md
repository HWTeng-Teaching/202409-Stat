8# **CH12.05_Q3.Q5** $\textcolor{orange}{黃家鈞}$
...
比較相同信賴水準(90%)與相同 $x_0$時，特別值的預測區間 (Prediction Interval) 一定比平均值的信賴區間 (Confidence Interval for the Mean Response) 寬。因預測區間除了考慮迴歸線估計的不確定性外（error），還包含獨特值本身的變異性（ε）。
